...
void run_asr(const char* audio_file, const char* params, char* grammar_id)
{
	...
	printf("\n语音识别结束\n");
	int tmp[200];
	sprintf(tmp,"/home/ubuntu/client  %s",rec_result);
	system(tmp);
...
{	
	int			ret						=	MSP_SUCCESS;
	const char* login_params			=	"appid = 58f64761, work_dir = ."; //登录参数,appid与msc库绑定,请勿随意改动
	/*
	* sub:             请求业务类型
	* result_type:     识别结果格式
	* result_encoding: 结果编码格式
	*/
	const char*	session_begin_params	=	"sub = asr, result_type = plain, result_encoding = utf8,sample_rate = 8000,aue = speex-wb";
	...
	printf("上传语法成功\n");
 	...
 	run_asr(argv[1], session_begin_params, grammar_id); //识别
